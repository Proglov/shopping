...
export default function SellerTX({ id }) {
    const { getAllTransActionsOfASeller } = Api
    ...
    useEffect(() => {
        const fetchTransactions = async () => {
            ...
            try {
                const response = await getAllTransActionsOfASeller({ page: currentPage, perPage, id });
                ...
                <Box className='flex flex-col gap-3'>

                    {
                        transactions.length === 0 ?
                            <span className="text-center">این فروشنده تاکنون سفارشی نداشته است!</span>
                            :
