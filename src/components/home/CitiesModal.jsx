...
export default function CitiesModal() {
    ...
    const handleConfirm = () => {
        ...
        provinces.map(province => province.cities.map(city => {
            if (city.isSelected) cityIds.push(city.cityId)
        }))
    ...
    const isAllCitiesSelected = cities => {
        for (let i = 0; i < cities.length; i++) {
            if (!cities[i].isSelected) return false
        }
        return true
    }
                            ...
                            <Accordion key={i * 223 + 5}>
                                ...
                                <AccordionDetails>
                                    <Typography className='flex justify-center'>
                                        <FormControlLabel dir='ltr' control={<Checkbox
                                            color='info'
                                    ...
                                    <Typography className='flex flex-col items-center'>
                                        {
                                            provinceObj.cities.map(cityObj => (
                                                <FormControlLabel key={cityObj.cityId} dir='ltr' control={<Checkbox checked={cityObj.isSelected} onChange={() => handleCheckboxChange(provinceObj.provinceId, cityObj.cityId)}
